...
def recursiveCopyInto(gauth, fID_from, fID_to, maxdepth=float('infinity'), __currentDepth=0):
    ...
    result = gauth.service.files().list(q='"%s" in parents and trashed = false' % fID_from, includeTeamDriveItems='true', supportsTeamDrives='true').execute()
    ...
    while True:
        ...
        for child in result['items']:
            ...
            if child['mimeType'] == 'application/vnd.google-apps.folder':
                ...
                exists_check = gauth.service.files().list(q='title = "%s" and "%s" in parents and trashed = false' % (child['title'].replace('"', '\\"'), fID_to), includeTeamDriveItems='true', supportsTeamDrives='true').execute()
                pp(exists_check)

                if exists_check['items'] == []:
                    print('  ' * (__currentDepth+1) + 'Trying to create folder "%s"' % child['title'])
                    new_folder = gauth.service.files().insert(body=data_new_folder, supportsTeamDrives='true').execute()
